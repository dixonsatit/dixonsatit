...
> ตัวอย่างนี้เป็นการกำหนดค่า array เข้าไปใน model เลย ซึ่งมันจะเหมาะกับข้อมูลที่ไม่ได้มีการเปลี่ยลแปลงบ่อยๆ ข้อมูลไม่เยอะ ไม่จำเป็นต้องสร้าง table เพิ่มเติม
...
เราจะทำการสร้าง function itemsAlias() เพื่อเก็บค่า array ทั้งหมดที่ต้องการใช้งานใน Model Resume ดังนี้

ในส่วนของ sex จะทำการประกาศเป็น constant เพื่อให้สามารถเรียกใช้งานด้วยชื่อง่ายๆ ไม่ต้องจำว่า 1 คือ ชาย, 2 คือ หญิง เวลาจะเรียกใช้งานก็ `Resume::SEX_MEN` เลย เพิ่มการกำหนดค่าเข้าไปใน Resume ดังนี้
...
หลังจากนั้นให้เราทำการสร้าง `static function itemsAlias()` เพื่อเก็บค่า array ทั้งหมดไว้ที่นี่ โดย สามารถเรียกใช้งาน และระบุค่า key ที่ต้องการเข้าไป ก็จะได้ array ของ items ที่เราต้องการ เช่น `Resume::itemsAlias('sex')` จะ return ค่า array sex ออกมา
...
จากนั้นสร้าง function เพื่อให้สามารถเรียกใช้งาน array ในฟอร์มได้ง่ายๆ เพื่อให้ฟอร์มนำไปแสดงผลเป็น DropdownList,RadioList,CheckBoxList
...
หลายๆ คนอาจสงสัยเราสร้าง itemsAlias ไว้แล้วทำไมยังต้องสร้าง getItem ขึ้นมาอีก จริงๆ สามารถเรียกใช้ itemsAlias ตรงๆได้ ตามตัวอย่างด้านล่างได้ จะเห็นว่ามันต้อง `use app\models\Resume` เพื่อให้สามารถเรียกใช้งาน function แบบ static แบบนี้ `Resume::itemsAlias('title')` ได้
...
ก่อนที่จะนำข้อมูลไปบันทึกเราจะแปลข้อมูลจาก checkBoxList ที่เป็น array ให้เป็น string โดยใช้ฟังก์ชัน `implode()` เพื่อให้เราไม่ต้องเขียนโค้ดวุ่นวายเราจะใช้ความสามารถของ  `behaviors()`  ของ model เพื่อดักจับ event `before-insert`,`before-update` เพื่อให้มันทำงาน ณ ตอนนั้น เพิ่มฟังก์ชันนี้เข้าไปใน Model Resume.php
...
{
   return $this->title.$this->firstname.' '.$model->lastname;
}
...
ตอนเรียกใช้งานก็ $model->sexLebel,$model->titleLebel,$model->skillLebel ได้เลย ส่วนใน gridView,DetailView ก็เหมือนตัวอย่าง fullName ด้านบน
